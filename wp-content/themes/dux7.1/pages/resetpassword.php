...
case 'lostpassword' :
	...
	if( _hui('yzm_on') && isset($_POST['ticket']) && isset($_POST['randstr']) && !hui_v_captcha($_POST['ticket'], $_POST['randstr']) ){
        $errors->add( 'yzmcuo', '验证码错误，请重新提交' );
    }else{
		if ( $http_post ) {
		...
		if ( isset( $_REQUEST['error'] ) ) {
			if ( 'invalidkey' == $_REQUEST['error'] )
				$errors->add( 'invalidkey', __( '密码重置链接无效' ) );
			elseif ( 'expiredkey' == $_REQUEST['error'] )
				$errors->add( 'expiredkey', __( '密码重置链接已过期，请重试' ) );
		}
	}
...
case 'resetpass' :
	...
	if ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] )
		$errors->add( 'password_reset_mismatch', __( '两次密码不一样' ) );
...
<div class="content-wrap">
	<div class="content resetpass">
		...
		<form action="<?php echo esc_url( mo_get_user_rp() . '?action=lostpassword' ); ?>" method="post">
			<?php errormsg($errors); ?>
			<h3>填写用户名或邮箱：</h3>
			<p><input type="text" name="user_login" class="form-control input-lg" placeholder="用户名或邮箱" autofocus required="required"></p>
			<p><input type="submit" value="获取密码重置邮件" class="btn btn-block btn-primary btn-lg" id="site_resetpassword"></p>
		</form>
		<?php 
			}else{ 
...
function retrieve_password() {
	...
	if ( empty( $_POST['user_login'] ) ) {
		$errors->add('empty_username', __('用户名或邮箱不能为空'));
	} else if ( strpos( $_POST['user_login'], '@' ) ) {
		$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
		if ( empty( $user_data ) )
			$errors->add('invalid_email', __('该邮箱不存在'));
	} else {
	...
	if ( !$user_data ) {
		$errors->add('invalidcombo', __('用户名或邮箱不存在'));
		return $errors;
	}
	...
	if ( ! $allow )
		return new WP_Error('no_password_reset', __('不允许此用户重置密码'));
	...
	$message = __('有人要求为以下帐户重置密码：') . "\r\n\r\n";
	$message .= network_home_url( '/' ) . "\r\n\r\n";
	$message .= sprintf(__('用户：%s'), $user_login) . "\r\n\r\n";
	$message .= __('如果不是你操作的，请忽略这封邮件！') . "\r\n\r\n";
	$message .= __('访问以下链接地址去重置密码：') . "\r\n\r\n";
